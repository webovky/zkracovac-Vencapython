...
def index():
    ...
    if shorcut and (url := Addresses.get(shorcut=shorcut).url):
        print(url)
    else:
        url, shocut = None, None
    if "nick" in session:
        user = User.get(nick=session["nick"])
        addresses = Addresses.select(lambda a: a.user == user)
        return render_template("base.html.j2", url=url, shorcut=shorcut, addresses = list(addresses))
    return render_template("base.html.j2", url=url, shorcut=shorcut)
...
def index_post():
    ...
    if url:
        ...
        address = Addresses.get(shorcut=shorcut)  # hledám jesli v DB náhodou takto zkratk až není
